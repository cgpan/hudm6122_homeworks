...
abline(a=0, b=1)
```  
The overall results showed that `TODO`.
...
## Exercise 1.5    
Here I write a function to get the Euclidean distance matrix for any shape of matrix. Since for any m * n matrix, there number of Eu-dist matrix's columns are always n-1+n-2+...+1. That is, there are n*(n-1)/2 columns in the Euclidean distance. 
```{r}
...
eudis_matrix<-function(a_matrix){
  ...
  eu_distance <- function(vec_1, vec_2){
    eu_dist <- sqrt(sum(vec_1-vec_2)^2)
    # this
    return(eu_dist)
    }
